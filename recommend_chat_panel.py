...
def open_recommend_chat_panel(parent: tk.Tk, recommend_result: dict, on_mouse_enter=None, center_dialog=None) -> None:
    dialog = tk.Toplevel(parent)
    dialog.title("AI 推荐对话")
    dialog.configure(bg=BG)
    dialog.attributes("-topmost", True)
    dialog.transient(parent)

    generated_at = str(recommend_result.get("market", {}).get("generated_at", "")).strip() or "latest"
    history_key = f"recommend::{generated_at}"
    history_map = _load_history_map()
    history: list[dict] = history_map.get(history_key, [])
    ...
    def sync_width(_event=None) -> None:
        width = max(240, canvas.winfo_width() - 6)
        ...
        for outer in messages_frame.winfo_children():
            for child in outer.winfo_children():
                if isinstance(child, tk.Label) and getattr(child, "_is_bubble", False):
                    child.configure(wraplength=max(220, width - 110))
    ...
    def append_bubble(role: str, content: str) -> None:
        ...
        bubble = tk.Label(
            ...
            text=content,
            justify="left",
            wraplength=max(220, canvas.winfo_width() - 110),
            anchor="w",
            padx=12,
            pady=10,
            fg=TEXT,
    ...
    if history:
        ...
        summary = recommend_result.get("content", "").strip()
        opener = "AI 已准备好，你可以围绕这次推荐继续追问。"
        if summary:
            opener += f"\n\n本轮总评：{summary}"
        append_bubble("assistant", opener)
